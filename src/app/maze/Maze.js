...
class Maze extends React.Component {
   ...
   onButtonClick = (row, col) => {
      ...
      this.array[row][col] =
         this.props.mazeItemValue === '' ? 's' : this.props.mazeItemValue
      this.setState({ array: this.array })
   ...
   onSolveClick = () => {
      ...
      this.setState({ arrayBackGroundColor })
   }
         ...
         <div>
            <div className="d-flex flex-row justify-content-center mb-12">
               {this.state.array.map((rowItem, rowIndex) => {
                  const columns = rowItem.map((colItem, colIndex) => {
                     return (
                        <button
                           key={uuidv4()}
                           style={{
                              ...
                              height: '50px',
                              backgroundColor:
                                 this.state.arrayBackGroundColor[rowIndex][
                                    colIndex
                                 ],
                           }}
                           onClick={() =>
                              this.onButtonClick(rowIndex, colIndex)
                           }
                        >
                           <span
                              hidden={this.state.axisHidden}
                              style={{ fontSize: '8px' }}
                           >
                              ${rowIndex}-${colIndex}
                           </span>
                           {this.state.array[rowIndex][colIndex]}
                        </button>
                     )
                  })
                  return (
            ...
            <div className="d-flex flex-row justify-content-center mb-12">
               ...
               <button
                  ...
                  type="button"
                  className="btn btn btn-primary-outline"
...
Maze.propTypes = {
   onMazeItemCallback: PropTypes.func,
   mazeItemValue: PropTypes.object,
